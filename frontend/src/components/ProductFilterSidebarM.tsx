...
const ProductFilterSidebarM: React.FC<ProductFilterSidebarProps> = ({
    onApplyFilter,
}) => {
    const [category, setCategory] = useState<string[]>([]);
    const [minPurchasePrice, setMinPurchasePrice] = useState<string>('');
    const [maxPurchasePrice, setMaxPurchasePrice] = useState<string>('');
    ...
    const generateFilterObject = (): {[key: string]: FilterValue} | void => {
        ...
        if (!isValidRange(minPurchasePrice, maxPurchasePrice)) {
            setError(
                'Мінімальна ціна закупу не може бути більшою за максимальну',
            );
            return;
        }
        if (!isValidRange(minSellingPrice, maxSellingPrice)) {
            setError(
                'Мінімальна ціна продажу не може бути більшою за максимальну',
            );
            return;
        }
        ...
        if (minPurchasePrice || maxPurchasePrice) {
            filter['purchase_price'] = {
                from: minPurchasePrice ? Number(minPurchasePrice) : undefined,
                to: maxPurchasePrice ? Number(maxPurchasePrice) : undefined,
            };
        }

        if (minSellingPrice || maxSellingPrice) {
            filter['selling_price'] = {
                from: minSellingPrice ? Number(minSellingPrice) : undefined,
                to: maxSellingPrice ? Number(maxSellingPrice) : undefined,
            };
        }
...
export default ProductFilterSidebarM;
